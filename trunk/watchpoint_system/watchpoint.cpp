...
int WatchPoint<ADDRESS, FLAGS>::end_thread(int32_t thread_id) {
   ...
   if (statistics_iter != statistics.end()) {                           // if thread_id is active
      ...
      #ifdef PAGE_TABLE
      Oracle<ADDRESS, FLAGS> temp_oracle = oracle_wp_iter->second;
      #endif
      ...
      #ifdef PAGE_TABLE
      page_table_wp.end_thread(thread_id);                              // end this thread in page_table
      watchpoint_t<ADDRESS, FLAGS> temp_wp = temp_oracle.start_traverse();  // start traversing through thread #1
      do {
         if (temp_wp.flags)
            page_table_wp.rm_watchpoint(temp_wp.start_addr, temp_wp.end_addr);
      } while (temp_oracle.continue_traverse(temp_wp) );
      #endif
...
void WatchPoint<ADDRESS, FLAGS>::reset() {
   ...
   #ifdef PAGE_TABLE
   page_table_wp.rm_watchpoint(0,-1);
   #endif
   return;
}
...
bool  WatchPoint<ADDRESS, FLAGS>::watch_fault(ADDRESS start, ADDRESS end, int32_t thread_id, bool ignore_statistics) {
   ...
   if (oracle_wp_iter != oracle_wp.end()) {                                // if thread_id found active
      ...
      #ifdef PAGE_TABLE
      bool page_table_fault = page_table_wp.watch_fault(start, end);
      #endif
...
int WatchPoint<ADDRESS, FLAGS>::set_watch(ADDRESS start, ADDRESS end, int32_t thread_id, bool ignore_statistics) {
   ...
   if (oracle_wp_iter != oracle_wp.end()) {                                   // if thread_id found
      ...
      #ifdef PAGE_TABLE
      page_table_wp.add_watchpoint(start, end, WA_READ | WA_WRITE);           // set page_table
      #endif
...
int WatchPoint<ADDRESS, FLAGS>::set_read(ADDRESS start, ADDRESS end, int32_t thread_id, bool ignore_statistics) {
   ...
   if (oracle_wp_iter != oracle_wp.end()) {
      ...
      #ifdef PAGE_TABLE
      page_table_wp.add_watchpoint(start, end, WA_READ);                      // set page_table
      page_table_wp.rm_watchpoint(start, end);              // rm WA_WRITE from page_table by checking the conjunction
      #endif
      if (!ignore_statistics) {
         statistics_iter = statistics.find(thread_id);
...
int WatchPoint<ADDRESS, FLAGS>::set_write(ADDRESS start, ADDRESS end, int32_t thread_id, bool ignore_statistics) {
   ...
   if (oracle_wp_iter != oracle_wp.end()) {
      ...
      #ifdef PAGE_TABLE
      page_table_wp.add_watchpoint(start, end, WA_WRITE);                     // set page_table
      page_table_wp.rm_watchpoint(start, end);              // rm WA_READ from page_table by checking the conjunction
      #endif
      if (!ignore_statistics) {
         statistics_iter = statistics.find(thread_id);
         statistics_iter->second.sets++;                                      // sets++ (if !ignored)
